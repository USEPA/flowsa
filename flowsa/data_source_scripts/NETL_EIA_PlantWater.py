...
def elci_parse(**kwargs):
  ...
  #  df['Water Type'] = df.loc[:, 'Water Type']
    ...
    surface = ['river', 'lake', 'reservoir', 'ocean', 'canal', 'aquifer', 'creek']
    total = ['municipal', 'wells']
    df['Compartment'] = ""
    for index, row in df.iterrows():
        if "lake wells" in df.iloc[index]['Water Type'].lower():
            df.iloc[index]['Compartment'] = "surface"
        elif "river water" in df.iloc[index]['Water Type'].lower():
            df.iloc[index]['Compartment'] = "total"
        elif "sewage" in df.iloc[index]['Water Type'].lower():
            df.iloc[index]['Compartment'] = "sewage"
        else:
            for sur in surface:
                if sur in df.iloc[index]['Water Type'].lower():
                    df.iloc[index]['Compartment'] = "surface"
            for tot in total:
                if tot in df.iloc[index]['Water Type'].lower():
                    df.iloc[index]['Compartment'] = "total"
