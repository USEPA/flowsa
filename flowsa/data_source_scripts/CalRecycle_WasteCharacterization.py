...
def calculate_flow_per_employee(fbs, **_):
    """
    Calculates FlowAmount per employee per year based on dataset name passed
    in "clean_parameter"
    clean_fbs function
    """
    bls = flowsa.flowby.get_flowby_from_config(
        name=fbs.config.get('clean_parameter')['name'],
        config={**{k: v for k, v in fbs.config.items()
                   if k in fbs.config['method_config_keys']},
                **fbs.config.get('clean_parameter')}
        )
    cols = ['Location', 'Year', 'SectorProducedBy']
    fbs = (fbs.merge(bls.rename(columns={'FlowAmount': 'Employees'})
                        .groupby(cols).agg({'Employees': 'sum'})
                        .reset_index(),
                     how='inner',
                     on = cols)
              .assign(FlowAmount = lambda x: 
                      np.divide(x['FlowAmount'], x['Employees'],
                                out=np.zeros_like(x['Employees']),
                                where=x['Employees'] != 0))
              .assign(Unit = lambda x: x['Unit'] + ' per employee')
